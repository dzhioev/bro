...
- `bro.py` — `BaseBro` ABC (the framework) and the framework helpers (`BroRaised`).
  ...
  `BaseBro` also auto-prepends every `prompts/shared/*.md` to the system prompt and appends a `## Data sources` block describing each declared `DataSource`. The bro's own class prompts (MRO-concatenated, before those additions) are kept as `persona`; `cw.py` injects it into `dive-in` sessions — currently only ppp-dev — so they carry that bro's policies without running under `--bro`.
...
- `bros/` — one package per concrete agent (`bros/<name>/__init__.py`); `bros/__init__.py:init()` registers each with the registry.
  ...
  - `devoops/` — autonomous operations: deploys, restarts, and infra inspection. Primitives are MCP tools in `infra/mcp.py` (`deploy`, `restart`, `ecs_status`, `probe`, `git_checkout`, `git_changes`, `list_targets`), plus the dev bro's `bash` tool for ad-hoc shell work; per-workflow procedure lives in `bro/bros/devoops/skills/*.md` (currently `deploy.md`) and is loaded on demand via the `skill` service tool
