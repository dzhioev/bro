...
- `bro.py` — `BaseBro` ABC (the framework) and the framework helpers (`BroRaised`).
  ...
  **Recording.** Sibling of the observer path: each `BaseBro.run()` / `.send()` also opens a *trail* via an `llm.tracker.Tracker` and emits framework bookend steps (`start_trail` → auto `system_prompt` step, `end_trail` → `end` step with the run's `end_reason`). The same `Tracker` is plumbed into the LLM (parallel to `observer`) so the provider implementation can emit the in-between steps directly (`user_input`, `llm_call` with raw request+response payload + token counts, `reasoning`, `assistant` with `terminal` flag, `tool_call`, `tool_result` with `is_error`). Default is `NullTracker` (no-op); per-run override via `tracker=` on `.run()` / `.send()`. The process-wide factory is swapped with `bro.bro.set_default_tracker_factory(...)` — `conftest.py` pins it to `NullTracker` for tests. `BaseBro._make_tracker()` is the per-bro hook. Trail schema and lifecycle live in `llm.tracker` and the framework revision is recorded as `configs.VERSION` on each trail header.
...
- `fork.py` — client-side fork of a recorded trail. `replay_messages(trail, up_to_step_id)` rebuilds the OpenAI input list from a `RecordedTrail`'s steps (system + user messages + each `llm_call.response.output` items + `function_call_output`s from `tool_result`s). `fork(parent_trail, up_to_step_id, *, llm_spec=None, system_prompt=None, record=True, tracker=None)` returns a fresh `Bro` preseeded with that prefix and a new trail bearing `parent=Parent(trail_id, step_id, 'fork')`; the caller drives it with `.send(next_message)`. The same-provider `previous_response_id` path is deferred to the trails-server stage
