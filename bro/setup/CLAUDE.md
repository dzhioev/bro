...
- `trails.json` — trails storage selection. `{ "backend": "local" }` stores under `$BRO_TRAILS_DIR/trails` when that env var is set, otherwise `$XDG_DATA_HOME/bro/trails` (default `~/.local/share/bro/trails`). `{ "backend": "service", "base_url": "https://trails.example", "token": "<bearer>" }` selects the HTTPS service; omitting `backend` from the existing `{ "base_url": ..., "token": ... }` shape also selects the service. The file remains required for production bro runs and every launch scope — `BaseBro`'s default tracker factory raises when it is missing rather than falling back. Claude recorder failures surface through `session-recorder-health.json` in the session's claude config dir (read by the cw statusLine and `cw banner`); daemon stderr is `session-recorder.log` beside it.
