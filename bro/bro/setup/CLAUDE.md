...
The framework repository is a uv workspace containing the `bro` and `bro-dev` distributions. `uv sync --all-packages --all-groups --all-extras` creates the shared root `.venv`, installs both members editably, and registers each distribution's committed console-script bridge. Each member owns its own pyproject tool configuration and development gate.
