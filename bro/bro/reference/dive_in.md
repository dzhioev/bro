...
This document explains the modes, workspace naming, the dispatcher-command seeding rules, and the rules around `--host`. The source of truth is `bro/bro/workflow/dive_in.py` (which also has unit tests in `dev/dive_in_test.py`).
