...
- `CW_BRO` — set by `start_session` when `--bro <name>` is passed; the container entrypoint reads it and runs `cw populate-bro-skills "$CW_BRO"` after venv activation to symlink the bro's skills into `.claude/skills/`. Also drives `cw banner`'s ASCII Bro logo + bro-name header.
