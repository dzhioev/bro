...
The container does **not** bind-mount `~/.claude.json` from the host, nor does the host's OAuth credentials file ever enter the container. Instead, `cw.py:_docker_create_argv` seeds a container-private `~/.claude.json` under `~/.claude/cw-sessions/<name>/` and bind-mounts that, while session auth comes from the `claude_code` token (below):
...
- `CW_COMMAND` — the user-visible `cw ss …` invocation, reconstructed by `start_session` (via `_cw_session_command`) for telemetry and the banner. Defaulted into `PPP_SHELL_COMMAND` if that's not already set.
