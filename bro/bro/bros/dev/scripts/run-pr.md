...
The `pr` argument carries the existing PR URL or number — e.g. `bro run ppp-dev '@:resume PR <pr-url>:@'` after the session that opened it died. Restore the state that session had, reconcile what happened while nobody watched, then rejoin the normal flow at the watcher (step 15).
...
2. **Recover the context** the environment no longer carries (`CW_TASK_ID` is unset here):
   ...
   ```
   - The task link is the `Task:` line in the PR body — use it for the task-logging steps (14, and `@::land`'s bookkeeping). No `Task:` line → proceed without task logging.
   - `<base>` is `baseRefName`.
...
4. **Reconcile unaddressed feedback from `gh` state — never trust a lost watcher.** The dead session may have died before, during, or after handling any event, and a restarted `poll-pr` baselines all existing events as already seen — feedback left unhandled now would be silently skipped forever. Pull the full review state:
   ...
   ```
   Treat as actionable any repo-owner feedback per step 16's rules that has no later reply from the PR author and no later commit addressing it; handle each per step 16. If the latest owner review is APPROVED and nothing actionable is pending, chain straight into `@::land` — no watcher needed.
5. **Resume watching**: continue at step 15.
...
No `run-tests` here: the full suite is the pre-push gate (step 10), run once on the final rebased tree — a pass before the rebase is evidence the rebase discards.
...
Conflicts → resolve them yourself, in-band: merge each conflict, `git add` the resolved paths, `git rebase --continue`. Then record the resolution on the task (same conditions as step 14): `brog::add_comment(task_id, topic='rebase conflicts', body=...)` naming the conflicted files and the resolution each one took — the pre-push gate (step 10) verifies the resolved result next.
...
### 10. Pre-push gate: `run-tests`

The flow's one mandatory suite pass — on the final rebased tree, immediately before anything is published:

```bash
run-tests
```

pyright + deptry + pytest + container smoke. **In a container session (`kind: container` from the `bro::banner` output), pass `--no-docker`** — the smoke step needs host Docker and will fail otherwise.{{when #harness = bro}} Run it with an explicit large `timeout_seconds` (600 fits) — `dev::bash`'s default kills a full suite mid-run; same for any other long command.{{end}}
...
Surface this link every time the PR enters review-pending: here at creation, and again after each push of review-fix commits (step 16).

### 14. Log "PR opened" to the task (sessions with a task)
...
2. Loop on `dev::watch(job_id, wait_seconds=1500)`. Each return is one iteration:
   - new output → react to every JSON line per step 16, then watch again;
   - a bare `running` state line (quiet window) → watch again;
   - `exited` right after a `merged`/`closed` event → the PR is terminal; react per step 16, stop looping;
...
**MUST launch via the `Monitor` tool with `persistent: true`. Do NOT use Bash `run_in_background`** — that only notifies on process exit, so review/comment events sit silently in the output file and approvals never trigger the auto-chain. The harness wakes you on each output event; react per step 16. Stop the watcher with `TaskStop` when chaining into `@::land`.
...
5. Run the pre-push gate (step 10).
...
7. Reply on the PR confirming the fix (reference the commit SHA):
   ...
   - **Reply to a specific review comment** (endpoint includes the PR number `<n>`):
     ...
     ```
8. Surface the review link again (step 13's `[#<n>](<pr-url>/files)` format) — the pushed commits put the PR back into review-pending.
...
**`conflicts` event**: rerun step 9 — the rebase, the in-band resolution default, the escalation bar, and the task comment all apply unchanged — then the pre-push gate (step 10), then push the rebased branch: `git push --force-with-lease origin HEAD` — the PR branch, never the base.
