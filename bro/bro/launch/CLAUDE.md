...
- `call.py` (`call`) — entry point for an interactive chat with a bro. By default opens the Textual chat UI in `call_tui.py` (IM-style: scrollable history, left/right bubbles, timestamp + date separators, animated "Typing…", Ctrl+D to quit, backtick (`) opens a stats modal). Falls back to raw mode (`[HH:MM:SS] bro: <reply>` lines + `> ` prompt) when stdin/stdout isn't a TTY; `--raw` forces it. The raw REPL helper `call_raw(bro, initial)` is library-callable. The Bro's `interactive=True` machinery picks up automatically in both modes: no `raise` tool, symmetric interactive-mode note injected into the system prompt
