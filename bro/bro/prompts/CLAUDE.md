...
Prompt content may carry `bro.base.template` directives (`#harness`/`#wire`/`#creds`; grammar and semantics: `bro/bro/reference/template.md`): every rendering surface renders its text once with its own facts via `bro.llm.mcp.render_text` — `BaseBro.__init__` for the two bro flavors, `bro/bro/cw/system_prompt.py:_session_append_prompt` for cw-sessions — so a directive works in `shared/` and bro class prompts alike. `FileSource`-served docs are the exception: one rendering is read by every harness, so their bodies must be surface-neutral — `FileSource.read` supplies no facts and a surface directive raises.
...
- `cw ss` picks by its `--hold` flag for both claude flavors — the cw-session append prompt and the `--raw` `--system-prompt` (flag semantics: `bro/bro/reference/cw.md`)
- the bro-native launch surfaces pick it through `bro/bro/bro.py:_system_prompt_for` — `run()` defaults unattended, `send()` guided, with every launcher's `--hold` overriding (per-surface defaults: `bro/bro/launch/CLAUDE.md`, "Launch holds")

`hold.md` (top level, not in `_BASE_PROMPT_FILES`) selects the per-level file in `holds/` via an exhaustive `{{iff #hold = …}}` chain and `{{include}}`s it; the three non-guided level files share `holds/authorization.md`, the full-authorization block. `bro.prompts.hold_fragment(hold, …facts)` is the one rendering path — every injection site uses it (`bro/bro/cw/system_prompt.py:_session_append_prompt`, `bro/bro/cw/claude_argv.py` for `--raw`, `bro/bro/bro.py:_system_prompt_for`), and it is the only call that supplies the `#hold` fact, so all other text stays hold-neutral mechanically: a stray `#hold` directive in a script or procedure doc raises.
...
- **Reference doc**: drop a `*.md` at top level or in a subdirectory (e.g. `dev/style.md`), then either add its filename to `bro/bro/cw/system_prompt.py:_BASE_PROMPT_FILES` (injected into every `cw ss` session) or declare a `FileSource` for it in `bro/bro/datasources/references.py` and mount it on a bro (tool-served on demand, every harness — e.g. `environment.md`)
