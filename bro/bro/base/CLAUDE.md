...
- `credentials.py` — client-side secret resolver (`__cli_name__ = 'credentials'`). `get` / `get_json` / `try_get` / `available` resolve a named secret against an ordered `Source` list (today only `local`, searching `<project>/.configs` then `~/.ppp`); a generated `credentials.json` overrides the built-in registry. `build_scoped_store` emits an in-memory per-container store (`cw` `docker cp`s it in), `apply_grant_revoke` layers per-session `--grant`/`--revoke` onto it, and `install_hooks` emits the shell wiring for secrets a tool reads from outside the resolver (git, the aws CLI). Schemas live in `setup/CLAUDE.md`.
