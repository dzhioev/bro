...
The schema evolves additively, never destructively. New fields arrive optional; readers (`trails` CLI, `TrailsClient`, `bro.fork`) tolerate their absence on old rows; stored rows are never migrated in place (both tables are append-only with indefinite retention). Every header carries `bro_version` (= `configs.VERSION` at write time) — a change that cannot be expressed additively bumps `configs.VERSION` and keeps readers working for every prior version, keyed off that field. The reserved-for-v2 sub-bro fields (`parent.relationship='subagent'`, `entry_point='subagent'`, `tool_result.child_trail_id`) follow the same rule: v1 rows simply omit them.
