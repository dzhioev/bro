...
`trails-repair-lineage` snapshots every Claude trail's UUID projection through the service, confirms duplicate candidates through the UUID index, and accounts for expected compatibility bodies still on legacy storage. `migrate` writes the exhaustive audit before calling manifested `relink`; `verify` repeats the clean store-wide audit and index lookup.
