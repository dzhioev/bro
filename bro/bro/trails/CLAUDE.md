...
The schema evolves additively, never destructively. New fields arrive optional; readers (`trails` CLI, `TrailsClient`, `bro.fork`) tolerate their absence on old rows; stored rows are never rewritten in place to change existing values (both tables are append-only with indefinite retention). The one sanctioned in-place write is an additive backfill that populates a newly-introduced optional attribute on old rows — idempotent, conditioned on the attribute's absence — when a new GSI needs it present to be complete; `server/backfill_gsi_pk.py` stamps the constant-PK `gsi_pk` behind the global newest-first list this way. Every header carries `bro_version` (= `configs.VERSION` at write time) — a change that cannot be expressed additively bumps `configs.VERSION` and keeps readers working for every prior version, keyed off that field. The reserved-for-v2 sub-bro fields (`parent.relationship='subagent'`, `entry_point='subagent'`, `tool_result.child_trail_id`) follow the same rule: v1 rows simply omit them.
