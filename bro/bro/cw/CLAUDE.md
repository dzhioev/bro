...
- `paths.py` — project-root / worktrees-dir / containers-dir resolution, `_venv_env`, `_in_container`, `_latest_jsonl`
...
- `docker.py` — thin docker wrappers (`running_mounts`, `find_container_id` — takes the mount `Path` so this stays a dependency-free leaf), image build (`_image_tag` / `_ensure_image`), `_docker_create_argv`, and the container-config constants it writes
...
- `containers.py` — container launch (`run_in_container`, `_seed_container_claude_json`, `_replace_container_resume_hint`) and `exec_in_workspace`
