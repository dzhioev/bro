...
_SUMMON_DESCRIPTION = (
  ...
  'the optional `grant` / `revoke` lists — entries are credential kinds, '
  f'`@bro` targets, or party permits ({summon.party_permit_choices()}). '
  'you can only grant what you hold '
  'yourself (a credential kind, a bro in your allow-list, or a permit). credential '
  'grants and revokes are idempotent; no-op `@bro` and permit changes remain strict. '
  'the same bound covers `harness` / `llm`: a '
