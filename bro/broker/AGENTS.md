...
— the wire format plus the transport that carries it.
...
`MAX_FRAME_BYTES` (1 MiB) and `PROTOCOL_VERSION` are the protocol constants in `brotocol.py`;
`ProtocolError` is the wire-violation exception (malformed JSON, oversize frame), raised by both the codec and the adapter.
...
- `bro.runtime.py` — the `Runtime`:
  ...
  Commands `spawn(launch, *, timeout)` / `expect(*, timeout)` / `send` / `kill` / `forget` / `serve` / `stop`;
  ...
  No topology, correlation, or protocol
  — a peer is its channel;
  the root is a uniform peer.
- `dispatcher.py` — the `Dispatcher` (logic layer) + the thin `Broker` facade + the built-in acceptance handlers.
  The `Dispatcher` is the Runtime's `Listener`;
  it owns the topology (`parent`/`children`),
  the correlation state (`origin[peer]`, `pending[in_reply_to]`),
  the `finalized` set,
  the handler registry (`on`),
  and the delivery observers (`add_delivery_observer`).
  `on_message` runs the four routing rules after a `finalized` drop-gate;
  `on_exit`/`on_timeout`/`on_gone`/a raising `spawn` launch synthesize `failed`;
  ...
  `ping_handler` + `spawn_test_handler` are the built-ins.
- `client.py` — the peer-side `Client` (synchronous, over `ClientTransport`):
  ...
  `send` / `request` / `call` / `await_reply` / `receive` / `close`.
  `request` and `call` are correlate-on-receive
  — they read until a message carries `in_reply_to == request.id`, setting uncorrelated arrivals aside for later `receive` calls;
  `TimeoutError` on deadline, `ConnectionError` on channel EOF.
  `request` returns the first correlated message;
  `call` rides through interims (`brotocol.INTERIM_TAGS`: `accepted` / `started`, surfaced to an `on_interim` callback) and returns the first correlated terminal, the whole call under one deadline.
  `send` returns the sent message (ids are minted client-side);
  `await_reply` is `call`'s wait detached from its send
  — with an opt-in `timeout_after_interim` that re-arms the deadline on each interim, bounding silence rather than the whole wait
  — and `await_any` is `request`'s (the first correlated message as-is, what an acceptance handshake reads), so a consumer can expose the request id before blocking;
  what `summon`'s blocking/`--detach`/`wait` modes ride on.
- `cli.py` — the `broker` console script:
  `send` / `request` / `receive` subcommands over `Client`.
...
- `broxy.py` — the peer-side broker proxy (`broxy` console script):
  ...
  Sticky per-request-id routing, a bounded in-order mailbox for waiter-gone messages, the local-only `claim{id}` / `check{id}` control messages.
...
- **Drain-before-decide.**
  ...
  — so a `completed` the child wrote just before exiting lands as `on_message` first and the Dispatcher finalizes on it.
...
- **The four routing rules, checked in order** (`on_message`, after the `finalized` drop-gate):
  (1) a message whose `in_reply_to` a peer awaits → that requester, as-is;
  (2) a spawned peer's lifecycle (`started`/`completed`) → its parent, re-tagged to the origin request id;
  (3) a fresh typed request → its registered handler
  — with `started`/`completed` handlers mounted, this is where the root's own lifecycle lands (it has no parent for rule 2);
  (4) anything else → refuse (v1 is parent/child-only, so an unroutable message is dropped + logged, not delivered)
  — except the root's unhandled lifecycle, dropped silently and un-finalized (the root's terminal is its exit, so its channel keeps serving the session).
- **`finalized` ⇒ exactly one terminal.**
  A peer is finalized when its `completed` is routed or a `failed` is synthesized for it;
  the set gates every later message from that peer (drop-after-terminal) and suppresses redundant synthesis, so whichever of `completed` / exit / timeout is processed first wins.
  `failed` is the only synthesized lifecycle
  — from `on_exit`-without-`completed` (`reason='exit'`),
  `on_timeout` (`reason='timeout'`, after the Runtime already killed the peer),
  `on_gone`-without-`completed` (`reason='disconnected'`, an expected peer's channel ended),
  or a `Dispatcher.spawn`/`expect` whose launch raised (`reason='launch'`, with the error string, correlated to the spawning request — no peer ever existed, so `finalized` is not involved).
  `Dispatcher.expect(parent, *, timeout, ready)` registers the external peer's topology exactly like a spawn and hands the provisioned channel to `ready` so the handler can publish the endpoint to whatever launches the peer.
- **Delivery tap + root exposure.**
  ...
  — rule 1, rule 2, and synthesized `failed`
  — as `(source, target, delivered message)`, with `source=None` for a launch failure;
  handler-driven `reply` and rule-4 refusals are not tapped.
  `root` exposes the root peer's channel (`None` until `run()` spawns it).
- **Uniform root, lock-free.**
  `run(root)` spawns the root like any peer (no request-lifecycle timeout) and returns its exit code on `on_exit`;
  the root has no origin, so no `failed` is synthesized for it
  — its exit just ends the session.
...
- **Sticky routes;
  the terminal ends the live exchange, not the conversation.**
  Every outbound message id maps to its sending connection;
  correlated inbound goes to exactly that connection.
  An interim keeps the route;
  the terminal — any correlated type outside `brotocol.INTERIM_TAGS`, mirroring `Client.call`
  — detaches the waiter, and the conversation stays retained for cursor reads until evicted.
  Route count is capped (`MAX_ROUTES`; eviction prefers detached-empty, then collected, then any detached route), since every fire-and-forget send mints an entry.
- **Mailbox:
  ...
  — collected first (terminal read), then oldest detached unread, never a live in-flight wait (the bound is exceeded rather than a wait broken);
...
- **Claim = a local stand-in request;
  the wait is a lock;
  collect is one-shot.**
  `claim{id}` (`Tag.CLAIM`) is never forwarded upstream.
  Unread messages are replayed
  — and future ones delivered
  — re-tagged to correlate to the claim message itself, so `Client.call('claim', {'id': ...})` behaves exactly like the original call did.
  Unknown id (never sent, or evicted) → immediate `reply{error}`;
  a collected conversation (terminal already read) → immediate `reply{error}` pointing at the cursor re-read;
  a claim while the current waiter's connection is alive → immediate `reply{error}` too (the newcomer fails fast, the live waiter keeps its route).
- **Check = the non-marking peek, or the cursor read.**
  `check{id, last_seen?}` (`Tag.CHECK`, local-only like claim) is always answered immediately and never supersedes a live waiter.
  Without `last_seen`:
  an unread terminal replays the unread window as copies re-tagged to the check, marking nothing (a later check or claim still finds it);
  otherwise `reply{state: 'pending'|'collected', seq}` (+ the trail id when a retained `started` carries one);
  an unknown id gets `reply{state: 'unknown'}`.
  With `last_seen: N`:
  ...
  the window is contiguous so the reader counts sequences from N;
  a window no terminal closes ends with a trailing `reply{state, seq}` marker;
  `last_seen` beyond `read_up_to` fails with "from the future" (it would acknowledge messages nobody has seen).
...
`brotocol_test.py` (codec round-trip + `v` field + malformed cases),
...
clean completed with drain ordering,
...
gone without a terminal,
kill closing the channel,
timeout-then-gone),
and `dispatcher_test.py` (the four rules + `finalized` + `failed`-synthesis against a fake `Runtime`:
ping reply correlation,
spawn topology registration,
child lifecycle re-tagged to the parent,
drop-after-terminal,
completed-then-exit collapsing to one terminal,
`failed{exit}` / `failed{timeout}` synthesis with the later exit deduped,
`failed{launch}` synthesis with no topology registered,
expect topology registration with the channel handed to `ready`,
expected-peer lifecycle routing with a post-terminal `on_gone` cleaning up,
`failed{disconnected}` synthesis,
rule-1 reply delivery,
rule-4 refuse,
root lifecycle (mounted handlers invoked, children unaffected;
dropped un-refused and un-finalized without them),
the delivery tap — rule-1/2 + synthesized `failed` observed,
`source=None` on launch failure,
handler replies untapped — root exposure,
...
`call` surfacing interims (`started`,
and `accepted` riding through while `await_any` returns it) and returning the first correlated terminal under one whole-call deadline,
`send` returning the client-minted request + `await_reply` reattaching to it,
the `timeout_after_interim` re-arm (a terminal outliving the initial bound / silence caught at a tighter one),
...
claim fail-fast on unknown/collected ids and on live-waiter collisions (the wait-is-a-lock rule),
claim re-await of a detached route,
the check states — unknown / pending with retained trail id + seq / pending across a read `accepted` / collected after claim / non-marking ready replay / live waiter undisturbed / malformed payload
— the cursor reads (post-delivery replay idempotence, read-marking spending the collect, pending window end-marker, from-the-future and malformed `last_seen` rejections),
...
No Docker.
`spawn.py` is ports-only (empty marker + ABCs), so it carries no test
