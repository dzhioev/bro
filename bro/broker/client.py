...
`request` and `call` are correlate-on-receive: they send a request, then read
inbound messages until one carries `request` naming the sent request's id.
`request` returns the first correlated message; `call` rides through progress
(surfaced to a callback) and returns the correlated result. Uncorrelated
...
peer emits them against the exchange id its launch carried (`EXCHANGE_ENV`).
"""
...
# the exchange a launched peer answers, set beside CHANNEL_ENV by whatever
# launches it (the host's spawner adapters, a manual summon's launch surface)
EXCHANGE_ENV = 'BROKER_EXCHANGE'
...
class Client:
  ...
  def progress(self, request_id: str, payload: dict[str, Any]) -> None:
    """emit a progress message on the exchange `request_id` — the answering side,
    for a worker peer."""
    self._transport.send(brotocol.progress(request_id, payload))

  def result(self, request_id: str, payload: dict[str, Any]) -> None:
    """emit the result closing the exchange `request_id` — the answering side, for
    a worker peer; `payload` is the result payload (`{outcome, value?, error?,
    detail?}`)."""
  ...
  def call(
    ...
    *,
    ...
    Correlated progress is surfaced to `on_interim` and the wait continues;
    the correlated result is returned. `timeout` bounds the whole call,
    progress included. Raises as `request` does.
    """
  ...
  def await_reply(
    ...
    *,
    ...
    `timeout_after_interim` is set: each correlated progress then re-arms the
  ...
  def await_any(self, request: Message, timeout: Optional[float]) -> Message:
    ...
    progress or result — the detached tail of `request`. Raises as it does."""
    deadline = time.monotonic() + timeout if timeout is not None else None
    return self._receive_correlated(request, deadline, timeout)
