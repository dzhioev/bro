...
@pytest.mark.asyncio
async def test_progress_and_result_emit_against_an_exchange(socket_dir):
...
@pytest.mark.asyncio
async def test_call_surfaces_progress_and_returns_the_result(socket_dir):
  async with running_server(socket_dir) as server:
    provisioned = await server.transport.provision()
    client = Client(UnixClientTransport(provisioned.host_endpoint))
...
@pytest.mark.asyncio
async def test_call_rides_every_progress_while_await_any_returns_the_first(socket_dir):
...
@pytest.mark.asyncio
async def test_call_without_callback_skips_progress_and_returns_failed(socket_dir):
  async with running_server(socket_dir) as server:
    provisioned = await server.transport.provision()
    client = Client(UnixClientTransport(provisioned.host_endpoint))
...
@pytest.mark.asyncio
async def test_call_deadline_spans_interim_progress(socket_dir):
...
@pytest.mark.asyncio
async def test_send_returns_the_sent_request(socket_dir):
  async with running_server(socket_dir) as server:
    provisioned = await server.transport.provision()
    client = Client(UnixClientTransport(provisioned.host_endpoint))
...
@pytest.mark.asyncio
async def test_await_reply_progress_rearms_the_deadline(socket_dir):
...
@pytest.mark.asyncio
async def test_await_reply_progress_rearm_shortens_a_longer_bound(socket_dir):
