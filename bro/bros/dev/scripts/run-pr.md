...
If the repo documents itself (`CLAUDE.md` files or an equivalent) and the change affects architecture, modules, commands, scripts, code style, or any documented section, update the docs. Bundle the update into the relevant commit, or make it its own commit if the docs change stands alone.
