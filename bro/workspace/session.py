...
`RIDE_RUNNER_PID` (its own pid) next to the harness process and forwards SIGTERM
...
def terminate_session() -> None:
  ...
  runner (RIDE_RUNNER_PID), which forwards the signal to the harness process and
