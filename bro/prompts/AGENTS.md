...
Prompt content may carry `bro.base.template` directives (`#harness`/`#wire`/`#creds`; grammar and semantics: `bro/reference/template.md`): every rendering surface renders its text once with its own facts via `bro.llm.mcp.render_text` — `BaseBro.__init__` for the two bro flavors, `ride/ride/claude/system_prompt.py:_session_append_prompt` for managed Claude sessions — so a directive works in `shared/` and bro class prompts alike. `FileSource`-served docs are the exception: one rendering is read by every harness, so their bodies must be surface-neutral — `FileSource.read` supplies no facts and a surface directive raises.
