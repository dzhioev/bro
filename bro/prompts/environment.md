...
4. `trail_id` is the trail this session is being recorded into — cite it whenever you record provenance about this session (a task you file, a hand-off note), and inspect it with `rewind show <trail-id>`. `trail_id: none (not published)` means recording is off or has not started yet; call the tool again later rather than treating it as settled.
