...
- `bro/trails/display/` owns the typed process-local display records, immutable scenario presets, stateful presentation core, renderer-neutral block operations, the `ObservedEvent` live adapter, and plain stream/retained plus lazy Rich-panel terminal renderers. It is a view layer only; none of its records enter trail storage or server contracts.
