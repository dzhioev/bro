...
- `network.py` owns `NetworkStore`, the authenticated wire proxy. HTTPS is required except for HTTP loopback hosts. It maps not-found, append-conflict, unsupported-operation, and transient transport failures onto the store errors and owns operation-specific retry schedules. Its concrete `recompute`, `check`, `relink`, and `repair_llm_spec` methods forward the Dynamo administration endpoints; they are not part of `TrailsStore`.
