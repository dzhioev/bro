...
- **Recorder placement:** `record/` owns the shared write spine and the framework's native recorder. A harness distribution owns its transcript acquisition daemon (`ride.claude.trail_recorder` for Claude); it may import the core store contract, schema, adapter, and write spine, never the reverse.
...
- `record/spine.py` owns blaze, ordinal extent validation, batched appends, liveness, and ending; `record/bro.py` adapts `bro.llm.tracker.Tracker`. `ride.claude.trail_recorder` records Claude transcripts, reporting each adopted segment's evidence and applying the core lineage verdict it gets back.
