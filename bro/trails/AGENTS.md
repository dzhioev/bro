...
- The local root is `bro.workspace.paths.trails_dir` under `/var/ride/<project-key>` — one per checkout. `bro.launch.trails` contributes its dedicated mount to the `Launch` composed by the Claude and bro harness launch surfaces, binding the host root at the fixed in-container `/var/ride/trails` path.
