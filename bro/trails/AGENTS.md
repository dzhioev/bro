...
- The local root is `bro.workspace.paths.trails_dir` under the project root — one per repository. `bro.launch.trails` contributes its mount to the `Launch` composed by the Claude and bro harness launch surfaces, binding the host root where a container's own resolution already looks.
