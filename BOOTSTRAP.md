...
You are the attended Claude Code session responsible for adopting the bro framework in a repository chosen by the user.
...
This bootstrap supports the full Claude Code harness only.
Do not install `bro-native`, recommend `--harness bro` or `--raw`, use Anthropic API-key authentication, or replace the exact acceptance command with host mode.
...
3. Determine the current Claude Code writable project boundary.
...
1. If this session is in a container, stop before any repository or credential write.
   Give the user a host restart command that opens Claude Code at the target root with the raw URL for this file.
...
4. Explain that `llm_spec` is the bro-native recipe inherited by non-Claude surfaces, while this full Claude session ignores it.
...
5. Read `git remote get-url origin` exactly.
   If it embeds userinfo or another credential, do not display or record it; propose replacing it with an approved credential-free SSH or HTTPS URL first.
6. Use the exact credential-free origin spelling as the portable project identity.
...
7. Use the canonical checkout path only when no remote exists or for a machine-specific override.
...
Before any issue write, run a read-only managed-session preflight using the configured developer and repository.
...
For a user token, inspect `repos/owner/name` for `has_issues` and repository role permissions.
...
Stop on an identity mismatch, disabled Issues, absent installation repository, incomplete metadata, or insufficient user permissions.

**Checkpoint 5:** report material names and modes without values, the exact origin identity, host-config selection layers, and required plus optional scope rows.
Also report command provenance and the verified managed-session GitHub identity plus repository access metadata.
...
After the identity and repository preflight from checkpoint 5 succeeds, run the configured developer in a managed session with a quoted heredoc or equivalent safe multiline input.
