...
from bro.summon import LiveChild, live_children, summoned, summoned_by_from_env
...
_LIVE_WORK_RULE = (
  ...
  "`bro::job('summon watch', mode='watch')` first); otherwise `bro::kill` each job and "
  '`bro::summon_cancel` each summon you no longer need, then end the turn. This notice comes '
...
class LiveWork:
  ...
  def keys(self) -> frozenset[str]:
    return frozenset(
      [*(job.id for job in self.jobs), *(child.request_id for child in self.children)]
    )
...
def live_work_notice(work: LiveWork) -> str:
  ...
  for child in work.children:
    lines.append(f'summon {child.request_id} to {child.target}')
...
class Runner:
  ...
  def _live_work(self) -> LiveWork:
    jobs = tuple(job.status() for job in self.registry.values() if job.is_live())
