...
class TestAttachInteractive:
  def _harness(self, monkeypatch, codes: list[int], running: list[bool]):
    ...
    monkeypatch.setattr(workspace_containers.subprocess, 'run', fake_run)
    monkeypatch.setattr(
      workspace_containers, 'container_running', lambda cid: next(running_iterator)
    )
    monkeypatch.setattr(
      workspace_containers, 'suspend_until_continued', lambda cid: events.append(('suspend', cid))
    )
    return events

  def test_detach_suspends_and_reattaches(self, monkeypatch):
    events = self._harness(monkeypatch, codes=[0, 4], running=[True])
    assert workspace_containers.attach_interactive('cid123') == 4
    assert events == [
  ...
  def test_container_exit_returns_without_suspend(self, monkeypatch):
    ...
    assert workspace_containers.attach_interactive('cid123') == 0
  ...
  def test_client_death_returns_without_probing_the_container(self, monkeypatch):
    ...
    assert workspace_containers.attach_interactive('cid123') == 130
    assert events == [['docker', 'start', '-a', '-i', '--detach-keys=ctrl-z', 'cid123']]
