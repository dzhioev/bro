...
class TestSessionAppendPrompt:
  def test_includes_base_prompts(self):
    out = ride_system_prompt.session_append_prompt('guided', 'bro')
  ...
  def test_non_guided_holds_add_the_authorization_block(self):
    for hold in ('unattended', 'detached', 'attended'):
      assert 'full authorization' in ride_system_prompt.session_append_prompt(hold, 'bro')
  ...
  def test_other_personas_carry_their_own_prompts_only(self):
    assert 'You are a software developer' not in ride_system_prompt.session_append_prompt(
      'guided', 'bro'
    )
