...
ending under `summon watch` with a summon in flight is a wait and stands. The
...
from bro.summon import LiveChild, live_children, summoned

Surface = Literal['full', 'raw']

WATCH_COMMAND = 'summon watch'
FAILURE_STATUS = 1
...
def _in_flight_lines(children: list[LiveChild]) -> str:
  return '\n'.join(f'summon {child.request_id} to {child.target}' for child in children)
...
def _unwatched_notice(children: list[LiveChild], surface: Surface) -> str:
  ...
  if surface == 'full':
    wait = (
      'Arm `Monitor` on exactly `summon watch`, persistent, and end the turn to wait for '
      'its events, or wait with `summon check --wait <request id>`; end a summon you no '
      'longer need with `summon cancel <request id>`.'
    )
  else:
    wait = (
      'Wait with `bro::summon_check(request_id, wait=true)` calls bounded under the call '
      'cap, or end a summon you no longer need with `bro::summon_cancel`.'
    )
  return (
