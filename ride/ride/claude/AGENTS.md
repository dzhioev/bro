...
- `print_anthropic_key.py`, `watch_guard.py`, and `stop_guard.py`
  ...
  print mode holds the process while a background task is pending, so it blocks a turn end once per turn only when summons are in flight with no `summon watch` armed or the watch is armed with nothing in flight,
