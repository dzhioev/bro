...
from bro.summon import LiveChild
...
class TestNotice:
  def test_summons_in_flight_without_a_watch_are_held_and_named(self):
    ...
    assert reason.startswith('2 summons in flight and no `summon watch` armed')
    assert 'Arm `Monitor` on exactly `summon watch`, persistent' in reason
    assert reason.endswith('summon 01m-child to bro-eyebro\nsummon 01m-other to bro')
  ...
  def test_a_raw_session_is_told_to_poll_instead_of_arming_a_monitor(self):
    ...
    assert reason is not None
    assert reason.startswith('1 summon in flight and no `summon watch` armed')
    assert '`bro::summon_check(request_id, wait=true)`' in reason
    assert 'Monitor' not in reason

  def test_a_watch_with_nothing_in_flight_is_held_with_its_task_named(self):
    payload = _payload(_task('summon watch', task_id='bf8'))

    reason = stop_guard.notice(payload, [], 'full', summoned=False)

    assert reason is not None
    assert reason.startswith('`summon watch` is armed (task bf8) with no summon in flight')
  ...
  def test_a_watch_over_summons_in_flight_is_the_wait_and_passes(self):
    assert (
      stop_guard.notice(_payload(_task('summon watch')), [_CHILD], 'full', summoned=False) is None
    )
  ...
  def test_a_watch_that_is_no_longer_running_does_not_count(self):
    payload = _payload(_task('summon watch', status='killed'))
  ...
  def test_the_stop_after_a_held_one_stands_whatever_is_live(self):
    payload = _payload(_task('summon watch'), stop_hook_active=True)
...
class TestMain:
  ...
  def test_a_held_turn_end_is_a_block_decision_on_stdout(self, monkeypatch, capsys):
    ...
    assert decision['reason'].endswith('summon 01m-child to bro-eyebro')

  def test_a_standing_turn_end_prints_nothing(self, monkeypatch, capsys):
    monkeypatch.setenv(CHANNEL_ENV, 'tcp://token@127.0.0.1:1')
    monkeypatch.setattr(stop_guard, 'live_children', lambda: [_CHILD])

    assert self._run(monkeypatch, capsys, _payload(_task('summon watch'))) == ''
  ...
  def test_without_a_broker_channel_the_journal_is_not_read(self, monkeypatch, capsys):
    ...
    out = self._run(monkeypatch, capsys, _payload(_task('summon watch', task_id='w1')))

    assert json.loads(out)['reason'].startswith('`summon watch` is armed (task w1)')
