...
class TestBuildSessionContext:
  def test_ride_session_records(self, tmp_path):
    records = build_session_context(
      system_prompt='injected text', bro='bro-dev', proj_root=_proj_with_instructions(tmp_path)
    )
    by = _by_kind(records)
    assert by['system_prompt']['subtype'] == 'ride_injected'
    assert by['system_prompt']['content'] == 'injected text'
    assert by['mcp']['fields'] == {'servers': ['persona:bro-dev']}
  ...
  def test_instructions_omitted_when_absent(self, tmp_path):
    records = build_session_context(system_prompt='x', bro='bro-dev', proj_root=tmp_path)
  ...
  def test_agents_md_wins_over_claude_md(self, tmp_path):
    ...
    records = build_session_context(
      system_prompt='x', bro='bro-dev', proj_root=_proj_with_instructions(tmp_path)
    )
