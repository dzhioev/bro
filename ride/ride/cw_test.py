...
class TestSsValidation:
  def test_ss_grant_with_host_is_accepted(self):
    ...
    with patch('ride.cw.start_session', return_value=0) as fake_start:
...
class TestResume:
  def test_dispatches_the_workspace_ref(self):
    with patch('ride.cw.resume_session', return_value=0) as fake_resume:
      rc = cw_cli.main(['cw', 'resume', 'c:w'])
    assert rc == 0
    assert fake_resume.call_args[0] == ('c:w',)
    assert fake_resume.call_args[1] == {'interface': 'cw', 'grant': [], 'revoke': []}

  def test_dispatches_the_scope_overrides(self):
    with patch('ride.cw.resume_session', return_value=0) as fake_resume:
      rc = cw_cli.main(['cw', 'resume', '--grant', '@dev', '--revoke', 'openai', 'w'])
    assert rc == 0
    assert fake_resume.call_args[1] == {'interface': 'cw', 'grant': ['@dev'], 'revoke': ['openai']}
...
class TestScope:
  def test_preserves_the_compatibility_surface_label(self):
    with patch('ride.scope_report.report_scope', return_value=0) as report:
      assert cw_cli.main(['cw', 'scope', '--raw']) == 0
...
class TestInPlace:
  def test_dispatches_to_the_runner(self):
    with (
      patch('ride.cw.run_in_place', return_value=0) as fake_run,
      patch('ride.cw.start_session') as fake_start,
    ):
  ...
  def test_hold_carried_in_the_inner_argv(self):
    ...
    # execution mode
    with patch('ride.cw.run_in_place', return_value=0) as fake_run:
  ...
  def test_skips_the_raw_gates(self):
    ...
    assert rc == 0
    assert options(fake_run.call_args[0][0]).raw
