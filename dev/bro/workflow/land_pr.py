#!/usr/bin/env python
"""merge the PR for the current branch in one shot, where its base permits.
...
def main(argv: list[str]) -> Optional[int]:
  parser = Parser(
    description='merge the PR for the current branch in one shot, where its base permits'
  )
  parser.add_argument(
    '--allow-unchecked',
    action='store_true',
